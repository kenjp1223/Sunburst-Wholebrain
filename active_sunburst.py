...
def create_dash_app(data_by_region, data_by_img, annotated_atlas_img, outputpath,
    ...
    def update_image(clickData, n_zm10, n_zm1, n_zp1, n_zp10, region_info):
        ...
        rgb_image_slice = color_code_image(data_slice, image_slice, IDs=np.array(region_info['IDs']), colormap=colormap)
    ...
    def export_figures(png_clicks, pdf_clicks, html_clicks, sunburst_fig_data, image_fig_data, region_info):
        ctx = dash.callback_context
        if not ctx.triggered or sunburst_fig_data is None or image_fig_data is None:
            raise dash.exceptions.PreventUpdate
        selected_acronym = region_info.get('selected_acronym', "unknown") if region_info else "unknown"
        ...
        if button_id == "export-png-button":
            ...
            image_filename = os.path.join(outputpath, f"image_{selected_acronym}.png")
            try:
                sunburst_fig.write_image(sunburst_filename, format="png", scale=high_res_scale)
                image_fig.write_image(image_filename, format="png", scale=high_res_scale)
                return f"PNG files saved: {sunburst_filename} and {image_filename}"
            except Exception as e:
                return f"Error saving PNG files: {e}"
        elif button_id == "export-pdf-button":
            sunburst_filename = os.path.join(outputpath, f"sunburst_{selected_acronym}.pdf")
            image_filename = os.path.join(outputpath, f"image_{selected_acronym}.pdf")
            try:
                sunburst_fig.write_image(sunburst_filename, format="pdf", scale=high_res_scale)
                image_fig.write_image(image_filename, format="pdf", scale=high_res_scale)
                return f"PDF files saved: {sunburst_filename} and {image_filename}"
            except Exception as e:
                return f"Error saving PDF files: {e}"
